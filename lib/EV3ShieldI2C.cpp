...
{
    if (!m_protocol)
        return BaseI2CDevice::readByte( location );
    else
...
{
    if (!m_protocol)
        return BaseI2CDevice::readInteger( location );
    else
        return SoftI2cMaster::readInteger( location );
}
...
{
    if (!m_protocol)
        return BaseI2CDevice::readLong( location );
    else
        return SoftI2cMaster::readLong( location );
}
...
{
    if (!m_protocol)
        return BaseI2CDevice::readRegisters(startRegister, bytes, buf);
    else
        return SoftI2cMaster::readRegisters(startRegister, bytes, buf);
}
...
{
    if (!m_protocol)
        return BaseI2CDevice::readString(location, bytes_to_read, buffer, buffer_length);
    else
        return SoftI2cMaster::readString(location, bytes_to_read, buffer, buffer_length);
}
...
{
    if (!m_protocol)
        return BaseI2CDevice::writeRegisters(start_register, bytes_to_write, buffer);
    else
        return SoftI2cMaster::writeRegisters(start_register, bytes_to_write, buffer);
}
...
{
    if (!m_protocol)
        return BaseI2CDevice::writeInteger(location, data);
    else
        return SoftI2cMaster::writeInteger(location, data);
}

bool EVShieldI2C::writeLong  (uint8_t location, uint32_t data)
{
    if (!m_protocol)
        return BaseI2CDevice::writeLong(location, data);
    else
        return SoftI2cMaster::writeLong(location, data);
}

uint8_t EVShieldI2C::getErrorCode  ( )
{
    if (!m_protocol)
        return BaseI2CDevice::getWriteErrorCode();
    else
        return SoftI2cMaster::getWriteErrorCode();
}

bool EVShieldI2C::checkAddress  ( )
{
    if (!m_protocol)
        return BaseI2CDevice::checkAddress();
    else
        return SoftI2cMaster::checkAddress();
}
